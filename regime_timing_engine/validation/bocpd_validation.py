"""
validation/bocpd_validation.py
=================================
...
局限说明（重要）：
  ...
  regime_assignment_validation.py 的工作），因此 hazard 函数暂用"全部历史
...
运行方式：
  python validation/bocpd_validation.py   (需先运行 ablation/01_data_loading.py, 02_feature_engineering.py, validation/duration_hazard_validation.py)
输出：
  outputs/validation/results/bocpd_results.csv
  outputs/validation/figures/bocpd_validation.png
"""
...
if __name__ == "__main__":
    ...
    out_df.to_csv(RESULTS_DIR / "bocpd_results.csv", index=False)
    print(f"\n结果已保存 -> {RESULTS_DIR / 'bocpd_results.csv'}")

    make_diagnostic_plot(out_df, FIGURES_DIR / "bocpd_validation.png")

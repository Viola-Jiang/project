...
（建议采用全收益口径）"，见 `pipeline/01_data_loading.py`）。真实数据没有
"哪天是牛市/哪天变盘"的上帝视角标签，因此 pipeline 03-06 与 ablation 的
...
4. **已知问题，暂不处理**：`ablation/run_ablation_summary.py`因为改动2
   ...
   显著增加（真实数据17年历史下单次完整S0~S5约15-20分钟）；K固定为3（未
   做数据驱动的自动选择）——这两项
...
├── data/                          # 真实中证800全收益数据（csi800_total_return.xlsx）+ pipeline/01、02生成的标准化中间文件
└── outputs/
...
```bash
python pipeline/01_data_loading.py          # 加载真实中证800全收益数据（data/csi800_total_return.xlsx）
...
### 真实数据实测结果（中证800全收益指数，2009-01-05~2026-07-09）
...
## `pipeline/` 组件验证的核心发现（真实数据：中证800全收益指数）

03/04/05/06 涉及的"区制转移点""区制标签"均指 `ref_regime`/`ref_regime_age`
（`engine/regime_labeling.py` 给出的自动标注参照，不是真值）。

| 脚本 | 验证内容 | 结论 |
|---|---|---|
| 01 | 真实数据加载 | 4253个交易日（2009-01-05~2026-07-09），无缺失/重复，标准化为[date, price] |
| 02 | 特征工程 + 自动标注参照标签 | z_t归一化确实压抑了跨区制波动差异（变异系数0.478→0.018） |
| 03 | 共轭发射模型 | 递归实现数值正确（误差<1e-13）；约51%自动标注参照变点表现出似然骤降 |
| 04 | 久期/hazard | 负二项hazard随年龄变化、几何hazard恒为常数，理论对比清晰坐实 |
| 05 | BOCPD引擎 | 数值正确；常数hazard下P(r_t=0)恒等于hazard本身（数学性质非bug）；检测率19.1% |
| 06 | 区制软分配 | 与自动标注参照的整体一致率50.2%；MAP段龄跟踪(<=5天占比)11.6% |

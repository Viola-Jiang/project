# 全天候指数仓位择时框架 —— 工程原型（合成数据验证阶段）

基于《贝叶斯在线变点检测与久期感知状态机》方法论报告的工程实现。
当前阶段使用**合成区制数据**（而非真实中证800）验证机制是否正确，
真实数据接入是下一阶段的工作。
...
│   ├── synthetic_data.py         # 合成三区制指数数据生成                  （文档§2, §6.5）
...
│   ├── calibration.py            # 因果walk-forward区制参数估计（不用真实标签）（文档§5.1, §5.4）
...
│   ├── 01_data_simulation.py         # 生成主线合成数据（种子42）           （文档§2, §6.5）
│   ├── 02_feature_engineering.py     # 特征工程正确性验证                  （文档§2）
...
│   ├── s5_multi_seed_robustness.py   # S5：多指数(多种子代理)×多窗口稳健性 + 统计去伪
...
├── data/                          # 合成价格与特征数据（由pipeline/01、02生成）
└── outputs/
...
| S5 | S4不变，改为多指数(多种子代理)×多时间窗(Purged K-Fold+Embargo)并行复算 + 统计去伪(BH-FDR, Deflated Sharpe) | 稳健性/泛化 |

**关于S1的HMM**：用 `hmmlearn.GaussianHMM` 在 `[z, log(realized_vol)]` 两维
特征上做EM估参+Viterbi解码，整段历史一次性喂入（刻意允许前视，这正是它
作为"理论上限参照"的意义）。

**关于S5的"多指数"**：本仓库目前只有一份基于中证800设想构造的合成数据，
没有真实多宽基/行业数据可用，因此用"多个独立随机种子生成的合成指数路径"
作为沙盒代理（`engine/synthetic_data.py` 的 `simulate(seed=...)`），真实
部署时应替换为真实的多宽基/行业数据，`ablation/s5_multi_seed_robustness.py`
的 docstring 里对这个近似有明确说明。

### 运行消融实验

```bash
python pipeline/01_data_simulation.py       # 先生成主线合成数据
...
| S5-多指数×多窗口(15次试验均值) | 0.22 | -14.5%(均值) | — | ✅ |

**S1→S2 前视偏差幅度 = 0.26个夏普点**：这是文档"主流离线HMM择时绩效部分
源于前视"这一核心主张的直接量化证据。
...
| §6.3 统计去伪（Deflated Sharpe Ratio; BH-FDR） | `engine/evaluation.deflated_sharpe_ratio` / `benjamini_hochberg` | 用于S5的15次独立试验 |
...
| 脚本 | 验证内容 | 结论 |
|---|---|---|
| 01 | 合成数据生成 | 三区制、负二项久期（非几何），久期/波动特征符合设计 |
| 02 | 特征工程 | z_t归一化确实压抑了跨区制波动差异（变异系数0.609→0.093） |
| 03 | 共轭发射模型 | 递归实现数值正确（误差<1e-13）；但z_t对多数区制切换的信号偏弱，只有约41%真实变点表现出似然骤降 |
